...
# generate RSD
docker run --rm -v ${output_dir}:${output_dir} -v ${ltf_raw}:${ltf_raw} blendernlp/covid-claim-radar:ke \
...
docker run --rm -v ${rsd_raw}:${rsd_raw} -v ${ltf_raw}:${ltf_raw} -v ${output_dir}:${output_dir} -v ${parent_child_tab}:${parent_child_tab} blendernlp/covid-claim-radar:ke \
    /opt/conda/envs/aida_tmp/bin/python /preprocessing/preprocess_detect_languages.py \
...
docker run --rm -v ${output_dir_ru}:${output_dir_ru} -v ${ltf_translated}:${ltf_translated} blendernlp/covid-claim-radar:ke \
    /opt/conda/envs/aida_tmp/bin/python /preprocessing/mt_converter.py \
    --lang_target ru \
