...
def ltf2bio_dir(ltf_input, output, ltf_filelist=None, separate_output=False):
    if os.path.exists(ltf_input):
        ltf_fp = []
        if os.path.isdir(ltf_input):
            if ltf_filelist:
                ltf_filelist = open(ltf_filelist).read().splitlines()
                ltf_fp = [os.path.join(ltf_input, item)
                        for item in ltf_filelist]
            else:
                ltf_fp = [os.path.join(ltf_input, item)
                        for item in os.listdir(ltf_input)
                        if '.ltf.xml' in item]
        else:
            ltf_fp = [ltf_input]
...
if __name__ == "__main__":
    ...
    ltf2bio_dir(ltf_input, output, ltf_filelist=ltf_filelist, separate_output=separate_output)
