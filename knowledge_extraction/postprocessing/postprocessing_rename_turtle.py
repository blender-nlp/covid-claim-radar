...
def get_column_idx(parent_child_file):
    try:
        for line in open(parent_child_file):
            ...
            parent_id_column = tabs.index("parent_uid")
            date_column = tabs.index("content_date")
...
def load_doc_root_mapping(parent_child_tab_path):
    child_column_idx, parent_column_idx, date_column = get_column_idx(parent_child_tab_path)
...
def load_root_doc_mapping(parent_child_tab_path):
    child_column_idx, parent_column_idx, date_column = get_column_idx(parent_child_tab_path)

    root_to_doc_dict = defaultdict(set)
    ...
    for one_line in f:
        ...
        root_id = one_line_list[parent_column_idx]  #[2] # parent_uid
        # root_id = one_line_list[7] # parent_uid
        root_to_doc_dict[root_id].add(doc_id)
    return root_to_doc_dict
...
if __name__ == '__main__':
    ...
    parser.add_argument('--parent_child_mapping_sorted', type=int,
