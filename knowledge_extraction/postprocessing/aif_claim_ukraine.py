...
def gen_ttl(claim_data, doc_ke, entity_info, evt_info, evt_args, evt_args_qnode, qnode_dict, 
    ...
    for doc_id in doc_ke:
        ...
        for entity_id in doc_ke[doc_id]['entity']:
            ...
            # # print(entity_type_cs) !!!!!!!!!!!!!! ukraine remove qnode
        ...
        for claim_idx, claim_dict in enumerate(claim_data[doc_id]):
            ...
            if claim_dict['start_char'] == 0:
                claim_start = sent_start
            else:
                claim_start = claim_dict['start_char'] + sent_start
            if claim_dict['end_char'] == 0:
                claim_end = sent_end
            else:
                claim_end = claim_dict['end_char'] + sent_start - 1
            if claim_end < claim_start:
                claim_end = sent_end
                # print(str(claim_dict))
            if "claim_span_text" in claim_dict and len(claim_dict["claim_span_text"]) > 0:
                claim_text = claim_dict["claim_span_text"]
                # print(doc_id, claim_dict['start_char'], claim_dict['end_char'], claim_dict['claim_span_text'])
                # print('-----------------------')
            ...
            # if 'qnode_x_varziable_identity' in claim_dict:
