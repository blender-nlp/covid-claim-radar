...
def get_sentences(path_to_txt, doc_id=None):
    if doc_id is None:
        doc_id = os.path.splitext(os.path.split(path_to_txt)[-1])[0]
    sent_tokenizer = PunktSentenceTokenizer()
    word_tokenizer = TreebankWordTokenizer()
    with open(path_to_txt) as fp:
        lines = [line.strip() for line in fp if len(line.strip()) > 0]
    sentences = []
    line_offset = 0
    for line in lines:
        sent_spans = sent_tokenizer.span_tokenize(line)
        for sent_span in sent_spans:
            sent_text = line[sent_span[0]:sent_span[1]]
            word_spans = word_tokenizer.span_tokenize(sent_text)
            tokens = []; token_offsets = []
            for s, e in word_spans:
                tokens.append(sent_text[s:e])
                token_offsets.append([line_offset+sent_span[0]+s,line_offset+sent_span[0]+e])
            sentences.append({"sent_id": f"{doc_id}-{len(sentences)}","sentence": sent_text, "tokens": tokens, "token_offsets": token_offsets})
