...
class BartPointerGen(PreTrainedModel):
    def __init__(self, config, tokenizer):
        super(BartPointerGen, self).__init__(config)
        self.config = config 
        ...
        #             words = [w for w in template.split(' ') if not re.match(r'"?<arg\d+>', w)]
        #             words_with_space = [' '+w for w in words]
        #             extra_vocab.update(words)
        #             extra_vocab.update(words_with_space)
        # self.extra_tokens = set() 

        # for w in extra_vocab:
        #     token_ids = tokenizer.encode(w, add_special_tokens=False)
        #     self.extra_tokens.update(token_ids)
    ...
    def convert_pointer_logits_to_lm_logits(self, pointer_logits, input_ids):
        ...
        # using scatter 
        # FIXME: scatter is wrong for duplicate indexes, but not using it get worse performance and speed 
        ...
        #         token_idx = input_ids[i,j]
        #         token_logits = pointer_logits[i, :, j]
        #         x = lm_logits[i, :, token_idx]
        #         x = torch.where(x > -1000, (x+token_logits), token_logits )
        #         lm_logits[i, :, token_idx] = x 
    ...
    def forward(self, input_ids, 
        ...
        #training 
            ...
            pointer_logits = torch.einsum('ijk,ilk->ijl', decoder_output, input_embeds) #(batch, seq_len, input_seq_len)
            # TODO: decrease <arg> outputs if neccesary 
